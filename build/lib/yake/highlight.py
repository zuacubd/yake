...
class TextHighlighter(object):
    ...
    def replace_token(self, text_tokens, y, n_gram_word_list):
        ...
        #modifyed the replacement: transform keyphrase "negative pressure rooms" to negativepressurerooms and
        #keep both the original keyphrase and transformed form.
        new_expression = txt.replace(re.sub('[!",:.;?()]$|^[!",:.;?()]|\W["!,:.;?()]', '',  txt), n_gram_word_list[0] + ' '+ self.highlight_pre + ''.join(n_gram_word_list[0].split(' ')) + self.highlight_post)
